...
#Cache for this web server       
Cache = None
latestCachedTime = None
...
class NewPost(Handler):
    ...
    def post(self):
        ...
        if subject and blog:
            ...
            b.put()
            #Not sure how I could append the newly inserted Blog to the Cache
            #Query database
            #Update Cache
            blogs = Blog.all().order('-created').fetch(limit = 10)
            logging.error('DB query !')
            Cache = blogs
            logging.info('Cache updated')

            latestCachedTime = datetime.datetime.now()
            logging.info('latestCachedTime=')
            logging.info (latestCachedTime)
...
class MainBlog(Handler):
    ...
    def render_front(self, blogs, timeInSecond):
        self.render("main_blog.html", blogs=blogs, timeInSecond=timeInSecond)
    ...
    def get(self):
        ...
        if not Cache:
            ...
            logging.info('Current Cache is found')
            blogs = Cache
            currentTime = datetime.datetime.now()
            if not latestCachedTime:
                latestCachedTime = currentTime
        
        logging.info("currentTime=")
        logging.info(currentTime)
        logging.info("latestCachedTime")
        logging.info(latestCachedTime)
        deltaTime = currentTime - latestCachedTime
        logging.error("deltaTime")
        logging.error(deltaTime)
            
        if (self.format == 'html'):
            self.render_front(blogs, deltaTime.seconds)
